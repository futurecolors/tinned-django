...
def create_dbs_and_users(developers, password):
...
def mysql_base_exists(user):
    sql = MYSQL_BASE_EXISTS.format(env.project_name, user)
    with settings(hide('warnings', 'running', 'stdout', 'stderr'), warn_only=True):
        result = mysql_execute(sql, 'root')
    return result.succeeded
...
def mysql_create_db(user):
    if mysql_base_exists(user):
        puts('База {0}_{1} уже существует. PASS'.format(env.project_name, user))
        return
    sql = MYSQL_CREATE_DB.format(env.project_name, user)
...
def mysql_grant_permissions(user):
    ...
    sql = MYSQL_GRANT_PERMISSIONS.format(db_name, env.project_name)
    mysql_execute(sql, 'root')
